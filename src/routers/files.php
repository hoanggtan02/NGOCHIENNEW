<?php
    ...
    $app->group($setting['manager']."/files", function($vars) use($app,$jatbi,$common,$setting,$account) {
    ...
    $app->router("/upload/{path}/{id}", 'GET', function($vars) use ($app, $jatbi,$account) {
        ob_start();
        $getType = $app->xss($_GET['type'] ?? '');
    ...
    $app->router("/upload-images", 'POST', function($vars) use ($app,$jatbi,$setting,$account) {
        $app->header([
            'Content-Type' => 'application/json',
        ]);
        $handle = $app->upload($_FILES['file']);
