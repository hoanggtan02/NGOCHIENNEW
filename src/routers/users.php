    ...
    $app->group($setting['manager']."/users" , function($app) use($jatbi,$setting){
        ...
        $app->router("/accounts", ['GET','POST'], function($vars) use ($app, $jatbi, $setting) {
            ...
            elseif ($app->method() === 'POST') {
                ...
                $app->doHookArgs('accounts-search', [&$where, &$join, &$columns]);
                $count = $app->count("accounts", $join, '@accounts.id', $where['AND']);
                $app->select("accounts", $join, $columns, $where, function ($data) use (&$datas,$jatbi,$app) {
                        $item = [
                            ...
                            "status" => $app->component("status",[
                                "url"=>"/users/accounts-status/".$data['active'],
                                "data"=>$data['status'],
                        ...
                        $args = [&$item, $data];
                        $app->doHookArgs('accounts-datas',$args);
                        $datas[] = $item;
                });
                echo json_encode([
                    "draw" => $draw,
        ...
        $app->router("/accounts-add", ['GET','POST'], function($vars) use ($app, $jatbi, $setting) {
            ...
            elseif ($app->method() === 'POST') {
                ...
                $checkaccount = $app->get("accounts", "account",["account"=>$app->xss($_POST['account']),"deleted"=>0,"ORDER"=>["id"=>"DESC"]]);
                if($app->xss($_POST['name'])=='' || $app->xss($_POST['account'])=='' || $app->xss($_POST['password'])=='' || $app->xss($_POST['status'])==''){
                    $error = ["status"=>"error","content"=>$jatbi->lang("Vui lòng không để trống")];
                }
                elseif($app->xss($_POST['account']) == $checkaccount){
                ...
                if(empty($error)){
                    ...
                    $app->doHook('POST-accounts-add-before-insert',$insert);
                    $app->insert("accounts",$insert);
                    $getID = $app->id();
                    $insert['id'] = $getID;
                    $app->doHook('POST-accounts-add-after-insert',$insert);
                    ...
                    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
                        ...
                        $imageUrl = '../public/assets/img/eclo.png';
                    }
        ...
        $app->router("/accounts-edit/{id}", ['GET','POST'], function($vars) use ($app, $jatbi, $setting) {
            ...
            elseif ($app->method() === 'POST') {
                ...
                if($data>1){
                    $checkaccount = '';
                    if($data['account']!=$app->xss($_POST['account'])){
                        $checkaccount = $app->get("accounts", "account",["account"=>$app->xss($_POST['account']),"ORDER"=>["id"=>"DESC"]]);
                    }
                    if($app->xss($_POST['name'])=='' || $app->xss($_POST['account'])=='' || $app->xss($_POST['status'])==''){
                        $error = ["status"=>"error","content"=>$jatbi->lang("Vui lòng không để trống")];
                    }
                    elseif($app->xss($_POST['account']) == $checkaccount){
                        $error = ['status'=>'error','content'=>$jatbi->lang('Tài khoản đã có người sử dụng')];
                    ...
                    if(empty($error)){
                        ...
                        $app->doHook('POST-accounts-edit-before-insert',$insert);
                        $app->update("accounts",$insert,["id"=>$data['id']]);
                        $getID = $data['id'];
                        $insert['id'] = $getID;
                        $app->doHook('POST-accounts-edit-after-insert',$insert);
                        if($_FILES['avatar']){
                            $imageUrl = $_FILES['avatar'];
                            $handle = $app->upload($imageUrl);
                            $path_upload = $setting['uploads'].'/'.$insert['active'].'/images/';
