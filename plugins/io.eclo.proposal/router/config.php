...
$app->group("/proposal/config", function ($app) use ($setting, $jatbi, $common, $template, $account, $stores) {
    $app->router("", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $account, $common) {
        if ($app->method() === 'GET') {
            ...
            $app->select("proposal_form", [
                ...
                $storeNames = array_map(function ($store) {
                    return $store['name'];
                }, $stores);
                $datas[] = [
    ...
    $app->router("/form-add", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $common, $stores) {
        ...
        if ($app->method() === 'GET') {
            ...
            if ($app->xss($_POST['name']) == '' || $app->xss($_POST['status']) == '' || $app->xss($_POST['form']) == '' || $app->xss($_POST['type']) == '') {
            ...
            if (empty($error)) {
                ...
                $getID = $app->id();
                // Giả sử form của bạn giờ đây gửi lên một mảng có key là 'stores'
    ...
    $app->router("/form-edit/{id}", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $setting, $common, $stores) {
    ...
    $app->router("/workflows", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $account, $common) {
        if ($app->method() === 'GET') {
            ...
            $app->select("proposal_workflows", [
                ...
                $forms_name = array_map(function ($form) {
                    return $form['name'];
                }, $forms);
                $stores = $app->select("stores_linkables", [
                    "[>]stores" => ["stores_linkables.stores" => "id",],
    ...
    $app->router("/workflows-add", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $common, $stores) {
    ...
    $app->router("/workflows-edit/{id}", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $setting, $common, $stores) {
        ...
        if ($app->method() === 'GET') {
            if ($data > 1) {
                ...
                $formSelect = array_map(function ($form) {
                    return $form['value'];
                }, $vars['forms']);
    ...
    $app->router("/target", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $account, $common) {
        if ($app->method() === 'GET') {
            ...
            $app->select("proposal_target", [
                ...
                $workflows_name = array_map(function ($workflow) {
    ...
    $app->router("/target-add", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $common, $stores) {
        ...
        if ($app->method() === 'GET') {
            ...
            if ($app->xss($_POST['name']) == '' || $app->xss($_POST['status']) == '' || empty($_POST['workflows'])) {
                $error = ["status" => "error", "content" => $jatbi->lang("Vui lòng không để trống")];
            }
            if (empty($error)) {
                $insert = [
    ...
    $app->router("/target-edit/{id}", ['GET', 'POST'], function ($vars) use ($app, $jatbi, $template, $setting, $common, $stores) {
        ...
        if ($app->method() === 'GET') {
            if (!empty($data)) {
                ...
                $vars['workflowsSelect'] = array_map(function ($workflows) {
