...
$app->group($setting['manager'] . "/invoices", function ($app) use ($jatbi, $setting, $stores, $accStore, $template) {
    ...
    $app->router('/sales/{action}', ['GET', 'POST'], function ($vars) use ($app, $jatbi, $setting, $stores, $accStore, $template) {
        ...
        // Kiểm tra và gán giá trị mặc định cho action
        $action = $vars['action'];

        $first = array_key_first($sales_session);

        // Xử lý khi có nhiều cửa hàng
        if (count($stores) > 1) {
            unset($_SESSION[$dispatch]);
        }

        // Kiểm tra thay đổi cửa hàng
        if (
        ...
        // Khởi tạo session nếu rỗng
        if (count($sales_session) == 0 && count($stores) == 1) {
            $action = 1;
            ...
            if (empty($sales_session[$action]['type_payments'])) {
                $sales_session[$action]['type_payments'] = $app->get("type_payments", "id", [
                    "status" => 'A',
                    "deleted" => 0,
                    "ORDER" => ["id" => "ASC"]
                ]);
            ...
            if (empty($sales_session[$action]['stores'])) {
                $sales_session[$action]['stores'] = $app->get("stores", "id", [
                    "id" => $accStore,
                    "status" => 'A',
                    "deleted" => 0,
                    "ORDER" => ["id" => "ASC"]
            ...
            if (empty($sales_session[$action]['point'])) {
                $sales_session[$action]['point'] = $app->get("point", "id", [
                    "status" => 'A',
                    "ORDER" => ["id" => "DESC"]
                ]);
            }
        ...
        // $SelectProducts = $_SESSION['sale']['add']['products'] ?? [];
        ...
        $branchs_data = $app->select("branch", ["id(value)", "name(text)"], ["deleted" => 0, "status" => 'A', "stores" => $accStore]);

        $vars['branchs'] = array_merge($empty_option, $branchs_data);
        $vars['type_payments'] = $app->select("type_payments", ["id", "name", "has", "debt"], ["deleted" => 0, "status" => 'A', "main" => 0]);
    ...
    $app->router('/invoices-update/{dispatch}/{action}/products/add/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting) {
        ...
        $get = $vars['get'];
        $sales_session = $app->getSession($dispatch) ?? [];
        $data = $app->get("products", "*", ["id" => $app->xss($get), "status" => "A", "deleted" => 0]);
        if ($data) {
            $sum = $data['amount'];
            $error = [];
            if ($data['amount_status'] == 1 && $sum <= 0) {
    ...
    $app->router('/invoices-update/{dispatch}/{action}/products/deleted/{get}', ['GET', 'POST'], function ($vars) use ($app, $jatbi, $setting) {
        if ($app->method() === 'GET') {
            ...
            if ($data) {
                ...
                if (empty($sales_session[$action]['vat'][$data['vat']])) {
    ...
    $app->router('/invoices-update/{dispatch}/{action}/products/price/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting) {
        ...
        $sales_session[$action]['products'][$get]['price'] = str_replace(',', '', $app->xss($_POST['value']));
        $app->setSession($dispatch, $sales_session);
        echo json_encode(['status' => 'success', 'content' => 'Cập nhật thành công']);
    })->setPermissions(['sales']);
    ...
    $app->router('/invoices-update/{dispatch}/{action}/products/amount/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting) {
        ...
        if (isset($_POST['value'])) {
            // Xử lý từ input số lượng
            $value = str_replace(',', '', $_POST['value']);
        } elseif (isset($_POST['increment'])) {
            // Xử lý từ nút tăng/giảm
            ...
            if ($_POST['increment'] === 'plus') {
                ...
                $value = max(0, $current_amount - 1); // Không cho số lượng âm
            }
        }
        ...
        if ($value < 0) {
            ...
            if ($dispatch == "returns") {
                $warehouses = $sales_session[$action]['products'][$get]['warehouses'];
                if ($value > $warehouses) {
                ...
                $getAmount = $app->get("products", ["id", "amount_status", "amount"], [
                    "id" => $sales_session[$action]['products'][$get]['products']
                ]);
                if ($getAmount['amount_status'] == 1) {
    ...
    $app->router('/invoices-update/{dispatch}/{action}/orderid/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $sales_session = $app->getSession($dispatch) ?? [];
        if (isset($active)) {
            $_SESSION['sales-active'] = $app->xss($get);
        }
        if (empty($sales_session[$action]['status'])) {
        ...
        if (empty($sales_session[$action]['type_payments'])) {
            $sales_session[$action]['type_payments'] = $app->get("type_payments", "id", ["status" => 'A', "deleted" => 0, "ORDER" => ["id" => "ASC"]]);
        }
        if (empty($sales_session[$action]['stores'])) {
            $sales_session[$action]['stores'] = $app->get("stores", "id", ["id" => $accStore, "status" => 'A', "deleted" => 0, "ORDER" => ["id" => "ASC"]]);
        }
        if (empty($sales_session[$action]['point'])) {
            $sales_session[$action]['point'] = $app->get("point", "id", ["status" => 'A', "ORDER" => ["id" => "DESC"]]);
        }
    ...
    $app->router('/invoices-updatee/{dispatch}/{orderkey}/cancel', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        unset($_SESSION[$dispatch][$orderkey]);
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/personnels', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $action = $vars['action'];
        $sales_session = $app->getSession($dispatch) ?? [];
        $data = $app->get("personnels", ["id", "name"], ["id" => $app->xss($_POST['value'])]);
        if ($data > 1) {
            ...
            $total_products = 0;
            $total_surcharge = 0;
            if (empty($sales_session[$action]['personnels'][$data['id']]["price"])) {
    ...
    $app->router('/invoices-update/{dispatch}/{action}/commission/{req}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $req = $vars['req'];
        $sales_session = $app->getSession($dispatch) ?? [];
        $sales_session[$action]['personnels'][$req]["price"] = str_replace(",", "", $app->xss($_POST['value']));
        $app->setSession($dispatch, $sales_session);
        echo json_encode(['status' => 'success', 'content' => 'Cập nhật thành công']);
    })->setPermissions(['sales']);
    ...
    $app->router('/invoices-update/{dispatch}/{action}/personnels-deleted/{req}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $action = $vars['action'];
        $req = $vars['req'];
        unset($_SESSION[$dispatch][$action]['personnels'][$req]);
        echo json_encode(['status' => 'success', 'content' => 'Cập nhật thành công']);
    })->setPermissions(['sales']);
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/branch', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $action = $vars['action'];
        $sales_session = $app->getSession($dispatch) ?? [];
        $data = $app->get("branch", "id", ["id" => $app->xss($_POST['value'])]);
        if ($data > 1) {
    ...
    $app->router('/invoices-update/{dispatch}/{action}/details-input/surcharge/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $sales_session[$action]['surcharge'][$get] = [
            ...
            "type"            => 'surcharge',
            "price"         => $app->xss(str_replace(['.', ','], '', $app->xss($_POST['value']))),
    ...
    $app->router('/invoices-update/{dispatch}/{action}/details/{req}', ['GET', 'POST'], function ($vars) use ($app, $jatbi, $setting, $accStore, $template) {
        if ($app->method() === 'GET') {
            ...
            $sales_session = $app->getSession($vars['dispatch']) ?? [];
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/products-details/{req}', ['GET', 'POST'], function ($vars) use ($app, $jatbi, $setting, $accStore, $template) {
        if ($app->method() === 'GET') {
            ...
            $req = $vars['req'];
            $sales_session = $app->getSession($dispatch) ?? [];
            $price_old = $_SESSION[$dispatch][$action]['products'][$req]['price_old'];
            if ($app->xss($_POST['type']) == 2) {
                $minus = $app->xss(str_replace([','], '', $_POST['minus']));
                $price = $price_old - $minus;
            } elseif ($app->xss($_POST['type']) == 3) {
                $precent = $app->xss(str_replace([','], '', $_POST['precent']));
                if ($precent > 100) {
                    $precent = 100;
                } elseif ($precent < 0) {
                    $precent = 0;
                ...
                $value = $app->xss(str_replace([','], '', $_POST['price']));
                $price = $app->xss(str_replace([','], '', $_POST['price']));
            }
            ...
            foreach ($sales_session[$action]['products'] as $value) {
                $total_products += (float)$value['amount'] * (float)$value['price'];
            }
            foreach ($sales_session[$action]['surcharge'] as $surcharge) {
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/discount', ['GET'], function ($vars) use ($app, $jatbi, $setting, $accStore, $template) {
        $vars['title'] = "Giảm giá";
        $sales_session = $app->getSession($vars['dispatch']) ?? [];
        $vars['data'] = $sales_session[$vars['action']]['discount'];
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/discount/add', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $sales_session[$action]['discount'] = $app->xss(str_replace([','], '', $_POST['value']));
        $app->setSession($dispatch, $sales_session);
        echo json_encode(['status' => 'success', 'content' => 'Cập nhật thành công']);
    })->setPermissions(['sales']);
    ...
    $app->router('/invoices-updatee/{dispatch}/{action}/details-prepay/{req}/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        if (
            ...
            count($sales_session[$action]['prepay'][$get]) == 0
        ) {
            $sales_session[$action]['prepay'][$get] = [
                "code"            => strtotime(date('Y-m-d H:i:s')),
                "type"            => 'prepay',
                $req         => $app->xss(str_replace(['.', ','], '', $_POST['value'])),
                "user"             => $app->getSession("accounts")['id'] ?? 0,
            ];
        } else {
            $sales_session[$action]['prepay'][$get][$req] = $app->xss(str_replace([','], '', $_POST['value']));
        }
    ...
    $app->router('/invoices-update/{dispatch}/{action}/details-deleted/{req}/{get}', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {
        ...
        $req = $vars['req'];
        // $sales_session = $app->getSession($dispatch) ?? [];
        unset($_SESSION[$dispatch][$action][$req][$get]);
    ...
    $app->router('/invoices-update/{dispatch}/{action}/completed/sales', ['POST'], function ($vars) use ($app, $jatbi, $setting, $accStore) {   
        ...
        $payments = ($total_products - $total_minus - $discount - $discount_customers) + $total_surcharge + $datas['transport']['price'];
        if ($datas['customers']['id'] == '' || count($datas['products']) == 0 || count($datas['personnels']) == 0) {
            $error = ["status" => 'error', 'content' => 'Lỗi trống'];
        }
        if (count($datas['prepay']) == 0) {
        ...
        // elseif($error_pre=='true'){
        // 	$error = ['status'=>'error','content'=>$lang['vui-long-nhap-hoa-hong']];
        // }
        if (!isset($error)) {
            $total = [];
            ...
            $insert = [
                ...
                "transport"        => $datas['transport']['price'],
                "payments"        => $payments,
                "user"            => $app->getSession("accounts")['id'] ?? 0,
                "date"            => date("Y-m-d"),
            ...
            if ($insert['stores'] > 0 && $insert['stores'] != '') {
                ...
                foreach ($datas['products'] as $value2) {
                    $warehouses_details = [
                        ...
                        "type"            => 'export',
                        "data"            => 'products',
                        "products"        => $value2['products'],
                        "price"            => $value2['price'],
                        "amount"        => $value2['amount'],
                        "amount_used"    => $value2['amount'],
                        "amount_total"    => $value2['amount'],
                        "stores"        => $insert['stores'],
                        "notes"            => $insert['notes'] == '' ? "Xuất bán hàng" : $insert['notes'],
                        "user"            => $app->getSession("accounts")['id'] ?? 0,
                        "date"            => date('Y-m-d H:i:s'),
                    ...
                    $warehouses_logs = [
                        ...
                        //"amount"=>$value2['buy'],
                        "amount" => $value2['amount'],
                        "total" => $value2['amount'] * $value2['price'],
                    ...
                    $total_cost = 0;
                    $total_cost += $warehouses_logs['total'];
                    $pro = [
                        "type"            => $datas['type'],
                        "invoices"        => $orderId,
                        "products"        => $value2['products'],
                        ...
                        "cost"            => $value2['cost'] ?? "",
                        "price"            => $value2['price'],
                        "price_vat"        => $value2['price_vat'],
                        "vat"            => $value2['vat'],
                        "vat_type"        => $value2['vat_type'],
                        "vat_price"        => $value2['vat_price'],
                        "price_old"        => $value2['price_old'],
                        "amount"        => $value2['amount'],
                        "warehouses"    => $value2['warehouses'],
                        "customers"        => $insert['customers'],
                        ...
                        "active"        => $jatbi->active(30),
                        "stores"        => $insert['stores'],
                        "discount"        => $value2['precent'] == '' ? 0 : $value2['precent'],
                        "discount_price" => $value2['precent'] * $value2['price_old'] / 100 == '' ? 0 : $value2['precent'] * $value2['price_old'] / 100,
                        "minus"            => $value2['minus'] == '' ? 0 : $value2['minus'],
                        "discount_invoices" => $datas['discount'] == '' ? 0 : $datas['discount'],
                        "discount_priceinvoices" => $datas['discount'] * $value2['price'] / 100 == '' ? 0 : $datas['discount'] * $value2['price'] / 100,
                        "branch"        => $app->get("products", "branch", ["id" => $value2['products']]),
                        "additional_information"        => $value2['additional_information'] ?? "",
                    ];
                    $app->insert("invoices_products", $pro);
                    $getpro_invoice = $app->id();
                    $pro_logs[] = $pro;
                    $getPro = $app->get("products", ["id", "amount"], ["id" => $value2['products']]);
                    $app->update("products", ["amount" => $getPro['amount'] - $pro['amount'],], ["id" => $getPro['id']]);
                }
                ...
                if ($datas['status'] == 1 && $datas['type'] == 2) {
                    ...
                    foreach ($datas['prepay'] as $prepay) {
                        ...
                        $prepay_payment = [
                            ...
                            "invoices"    => $orderId,
                            "type"        => "prepay",
                            "price"     => count($datas['prepay']) == 1 ? $insert['payments'] : str_replace([','], '', $prepay['price']),
                            "content"     => $app->xss($insert['notes']),
                            "date"         => date('Y-m-d H:i:s', strtotime($app->xss($insert['date']))),
                            "user"        => $app->getSession("accounts")['id'] ?? 0,
                            "date_poster" => date('Y-m-d H:i:s'),
                            "stores"    => $insert['stores'],
                        ];
                ...
                if ($datas['status'] == '1' && $datas['type'] == 1) {
                    //if($insert['payments']>0){
                    // foreach ($datas['type_payments'] as $prepay) {
                    // if($prepay['price']==''){
                    $getType_Payment3 = $app->get("type_payments", ["id", "code", "debt", "has"], ["id" => $datas['type_payments']]);
                    $prepay_payment3 = [
                        "type_payments"    => $getType_Payment3['id'],
                        "code"        => strtotime(date('Y-m-d H:i:s')),
                        "invoices"    => $orderId,
                        "type"        => "prepay",
                        "price"     => $app->xss($insert['payments']),
                    ...
                    $app->update("invoices", $update3, ["id" => $orderId]);
                    $expenditure3 = [
                        "type"             => $expenditure_type,
                        "debt"             => $app->xss($getType_Payment3['debt']),
                        "has"             => $app->xss($getType_Payment3['has']),
                        "price"         => $expenditure_code . $insert['payments'],
                        "content"         => $app->xss($insert['code']),
                        "date"             => $app->xss($insert['date']),
                ...
                if ($datas['status'] == '2' && $datas['type'] == 1) {
                    ...
                    // foreach ($datas['type_payments'] as $prepay) {
                    // if($prepay['price']==''){
                    $getType_Payment4 = $app->get("type_payments", ["id", "code", "debt", "has"], ["id" => $datas['type_payments']]);
                    $prepay_payment4 = [
            ...
            // $jatbi->logs('invoices','add',[$insert,$pro_logs,$datas,$transports],$orderId);
            unset($_SESSION[$dispatch][$action]);
